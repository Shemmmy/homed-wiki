...
По сути, значение этого поля является частью топика [`td`](/common/topics/#td-to-device), относящейся к конкретному устройству.
...
Условия для выполнения действий могут определяться не только специальным действием [`condition`](#condition), а также тем, какой конкретно триггер вызвал срабатывание автоматизации, при условии, что для триггеров заданы [имена](/automation/database/triggers/#_2).
